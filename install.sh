...
cp_conflict() {
  ...
  while true; do
    ...
    O)
      ...
      [ "$is_dir" = "dir" ] && cp -r "$src" "$dst" || cp "$src" "$dst"
      info "Overwritten: $dst (overwrite-all active)"
      return
      ;;
    S)
      _DEPLOY_ALL="skip"
      warn "Skipped: $dst (skip-all active)"
      return
      ;;
...
if [ -d "$HOME/.config/bspwm" ] || [ -d "$HOME/.bspwminstaller" ]; then
  ...
  1)
    ...
    info "Mode: overwrite (backup → replace all)"
    ;;
  2)
    INSTALL_MODE="merge"
    info "Mode: merge (add new, ask on conflicts)"
    ;;
  3)
    INSTALL_MODE="add"
    info "Mode: add (missing files only)"
    ;;
  4)
    echo "Aborted."
    exit 0
    ;;
  *)
    warn "Invalid choice — defaulting to add (safest)."
    INSTALL_MODE="add"
    ;;
  esac
  echo ""
...
if [ "$INSTALL_MODE" = "overwrite" ]; then
  ...
  for cfg in bspwm neovim alacritty kitty clipcat gtk-3.0 mpd ncmpcpp paru yazi btop fastfetch logtime Thunar; do
